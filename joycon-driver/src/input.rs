...
impl From<&[u8]> for InputReport {
    fn from(buf: &[u8]) -> InputReport {
        match buf[0] {
            ...
            0x3f => SimpleInput(LittleEndian::read_u16(&buf[1..3]), buf[3]),
            _ => Unknown,
        }
    }
}
...
impl From<&[u8]> for ResponseData {
    fn from(buf: &[u8]) -> ResponseData {
        match buf[1] {
            ...
            0x10 => ResponseData::ReadFlash(FlashChunk::from(&buf[2..])),
...
pub struct FlashChunk(pub u16, pub Vec<u8>);

impl From<&[u8]> for FlashChunk {
    fn from(buf: &[u8]) -> FlashChunk {
        let addr = LittleEndian::read_u16(&buf[..4]);
        let size = buf[4] as usize;
        let buf = &buf[5..5 + size];
        FlashChunk(addr, Vec::from(buf))
    }
}
